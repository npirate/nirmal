...
c_pos_list = [] #could also use list() function
...
while True: #true is never a reason to stop...will go on forever
  ...
  if new_item == 0:
      ...
      c_pos_list.append (new_item)
  continue #continues the loop/starts over

print ('Configured POS codes are: ', c_pos_list)
...
for everyline in filedata:
  if everyline[0] not in ['E','F','O','R']:# and everyline[1] == '56420':
    ...
    if cpt not in uaub_op:
      c_pos_list = [pos for pos in c_pos_list if pos not in c_pos_op_uaub_list]
      c_pos_op_uaub = 0
    else:
    ...
    if cpt not in uaub_ip:
      c_pos_list = [pos for pos in c_pos_list if pos not in c_pos_ip_uaub_list]
      c_pos_ip_uaub = 0
    else:
    ...
    if errate == '--':
      if cutbackind == '0':
        pos_list = [] #removed pos that were configured for er and cutback since code does not have those rates
      else:
        pos_list.extend(c_pos_list)
        pos_list.remove(23) #removed only er pos
    else:
      if cutbackind == '0':
        pos_list.append(23)#removed cutback pos
      else:
    ...
    if configured_pos is not 0:
      pos_medi_cal = configured_pos - len(c_pos_op_uaub_list) if cpt in uaub_op else configured_pos
      pos_medi_cal = pos_medi_cal - len(c_pos_ip_uaub_list) if cpt in uaub_ip else pos_medi_cal
    else:
      pos_medi_cal = 0
