...
train = train.drop(['runtime','gender','expensive','money_back_guarantee'], axis = 1)
...
train = pd.get_dummies(train,prefix = ['industry', 'genre', 'airtime'], columns = ['industry', 'genre', 'airtime'])
...
train = train.drop(['ratings','relationship','industry_Other','genre_Direct','airtime_Daytime','airlocation'], axis = 1)
...
test = test.drop(['runtime','gender','expensive','money_back_guarantee'], axis = 1)
...
#doing one hot encoding
test = pd.get_dummies(test,prefix = ['industry', 'genre', 'airtime'], columns = ['industry', 'genre', 'airtime'])
...
#dropping one dummy variable of each feature to avoid dummy variable trap
test = test.drop(['ratings','relationship','industry_Other','genre_Direct','airtime_Daytime','airlocation'], axis = 1)
...
result_submission.to_csv('submission10.csv', index = False)#indexes are not copied in the csv
